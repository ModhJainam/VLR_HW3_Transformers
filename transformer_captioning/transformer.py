...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        dot_product = torch.bmm(query, key.transpose(1,2)) / np.sqrt(self.embed_dim)
        ...
        if attn_mask is not None:
            ...
            additive_mask = -1e10 * (1 - attn_mask)
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = self.query_proj(query).view(N, S, H, D//H).transpose(1,2) #(N, H, S, D/H)
        key = self.key_proj(key).view(N, T, H, D//H).transpose(1,2) #(N, H, T, D/H)
        value = self.value_proj(value).view(N, T, H, D//H).transpose(1,2) #(N, H, T, D/H)
        ...
        dot_product = torch.matmul(query, key.transpose(2,3)) / np.sqrt(self.embed_dim)

        if attn_mask is not None:
            # convert att_mask which is multiplicative, to an additive mask
            # Hint : If mask[i,j] = 0, we want softmax(QKT[i,j] + additive_mask[i,j]) to be 0
            # Think about what inputs make softmax 0.
            additive_mask = -1e10 * (1 - attn_mask.float())
            dot_product += additive_mask
